...
## What Results contains — and what to move out
...
- **Finding-first, not narrative-first.** Open every subsection and paragraph with the
  ...
  possible". Prefer result-stating openers — "The results show…", "Under the baseline
  scenario…", "Across sensitivity cases…", "This estimate decreases / increases under…", "The
  spatial distribution is concentrated in…" — over interpretive or promotional ones — "This
  reveals…", "This highlights the importance of…", "This opens a new pathway…", "This suggests
  a paradigm shift…", "This provides a bridge to…".
...
## How it reads — language and voice
...
- **Objective voice, present tense.** Largely objective/passive: "the analysis reveals",
  "these results indicate". **Avoid repeated "we find / we observe"** (per `00`) — say what
  the data show, not that you saw it; use "we quantify / we find" only sparingly. Present
  tense for patterns and their interpretation.

## Bounding claims — scope and limits
...
**Describe-a-phenomenon template (the arc in one sentence).** The same five beats compress to:
...
- **Content scope:** Does each paragraph open with a finding (not a motivation)? Any metaphor,
  slogan or "why it matters" claim to delete or move to the Discussion?
- **Language and voice:** Rhetoric and over-strong terms downgraded? Association language
  unless causality is identified? "we" near zero, objective voice, present tense?
- **Bounding claims:** Does every strong claim name its scope (scenario, unit, horizon,
  boundary)? Are scope limits folded *into* the result sentence (metric name, comparison
  object, boundary phrase, subordinate clause) rather than stated as standalone "does not
